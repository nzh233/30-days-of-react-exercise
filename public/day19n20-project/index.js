...
const DisplayCard = ({
  ...
  useEffect(() => {
    getRes(`https://api.thecatapi.com/v1/images/search?breed_id=${id}`).then(
      (res) => {
    ...
    <div id="header-wrapper">
      ...
      <Router>
        ...
        <Route
          ...
          component={() => {
            changeDay("day-20");
            return <Link to="/day-20">Day 20 &gt;&gt;</Link>;
          }}
        />
        <Route
          path="/day-20"
          component={() => {
            changeDay("day-19");
            return <Link to="/day-19">&lt;&lt; Day 19</Link>;
          }}
        />
      </Router>
      <img
      ...
      <p>
        There are <span className="strong-number">{avgWeight}</span> Kg and
        lives <span className="strong-number">{avgLifeSpan}</span> years on
        average.
      </p>
    </div>
  );
...
const Body = ({ catData, day, buttons }) => {
  return (
    <div id="body">
      <div className="sort-tags">
        {day === "day-19" &&
          buttons.map(({ text, callback }) => {
            return (
              <Button type="button" onClick={callback} key={`btn-${text}`}>
                {text}
              </Button>
            );
          })}
      ...
      {catData.map((data) => {
        return <DisplayCard key={data.id} catInfo={data} />;
      })}
    </div>
  );
};
...
const Footer = () => {
  return (
    <div id="footer-wrapper">
      ...
      <p>
        Built by <a href="https://github.com/nzh233">nzh233</a>
      </p>
    </div>
  );
};
  ...
  return (
    <>
      <Header
        ...
        changeDay={changeDay}
      />
      <Body catData={showedData} buttons={buttons} day={day} />
      <Footer />
    </>
  );
};
